...
corpus=corpus_Data(tagged_data)

tf_idf_vect = TfidfVectorizer()
final_tf_idf  = tf_idf_vect.fit_transform(corpus)
dictionary=dict(zip(tf_idf_vect.get_feature_names(),list(tf_idf_vect.idf_)))  ### mapping word:idf (optional)
tfidf_feat = tf_idf_vect.get_feature_names()
#tfidf = final_tf_idf[row, tfidf_feat.index(word)]

############ Crmbining Doc2vec + tfidf ################
tfidf_sent_vectors_ft = [] 
row=0
for article in corpus: # for each review/sentence
    sent_vec = np.zeros(300) # as word vectors are of zero length
    weight_sum =0; # num of words with a valid vector in the sentence/review
    article=word_tokenize(article)
    for word in article: # for each word in a review/sentence
        vec = model_d2v.wv[word]
        # obtain the tf_idfidf of a word in a sentence/review
        try:
            #tfidf = final_tf_idf[row, tfidf_feat.index(word)]
            tfidf= dictionary[word]*(article.count(word)/len(article))
        except:
            tfidf=0
        sent_vec += (vec * tfidf)
        weight_sum += tfidf
...
for i in range(len(document_embeddings)):
    ...
    document_embeddings[i]=tfidf_sent_vectors_ft[i]
