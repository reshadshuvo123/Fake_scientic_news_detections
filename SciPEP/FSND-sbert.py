...
news_texts = news_title + ' ' + article_text
abstract_texts = [non_duplicate_titles[i] + ' ' + non_duplicate_abstract[i] for i in range(len(non_duplicate_abstract))]

news_texts=sent_tokenize(news_texts)
news_embeddings = model.encode(news_texts, convert_to_tensor=True)
news_embeddings= torch.mean(news_embeddings, axis=0)

extracted_abstract_score=[]
for i in range(len(abstract_texts)):
    abstract=sent_tokenize(abstract_texts[i])
    abstract_embedding = model.encode(abstract, convert_to_tensor=True)
    abstract_embedding = torch.mean(abstract_embedding, axis=0) 
    cosine_scores = util.pytorch_cos_sim(news_embeddings, abstract_embedding)
    extracted_abstract_score.append(cosine_scores)
