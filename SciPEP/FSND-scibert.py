...
#26
#ink='https://www.nature.com/articles/d41586-019-03334-5'
#27
...
print('\n')
print('Ranking based on Cosine similarity : using Scibert-sentece embedding ')
...
def avg_h(text):
    ...
    for i in range(len(text)):
        if len(text[i])>510:
            news= embed_text(text[i][:510], model).mean(1)
            news=news.detach().numpy()
        else:
            news= embed_text(text[i], model).mean(1)
            news=news.detach().numpy()
        scibert_emd.append(news)
    news_embd=np.mean(scibert_emd,axis=0)
    return news_embd
...
def avg_embd(papers):
    ...
    for i in range(len(papers)):
        print('***********')
        print(papers[i])
        c=embed_text(papers[i], model).mean(1)
        em.append(c.detach().numpy())
        doc2vec=np.mean(em,axis=0)
    return doc2vec

news_article = aveg_embd(text)

extracted_abstract_score=[]

for i in range(len(non_duplicate_abstract)):
    papers=non_duplicate_abstract[i].replace("\n", "")
    papers=sent_tokenize(papers)
    news = aveg_embd(papers)
    cosine = cosine_similarity(news_article, news)
