...
print('\n')
print('Ranking based on Cosine similarity : XLnet ')
...
paper_texts = [non_duplicate_titles[i] + ' ' + non_duplicate_abstract[i] for i in range(len(non_duplicate_abstract))]

#Compute embeddings for all papers
corpus_embeddings = model1.encode(paper_texts, convert_to_tensor=True)

#We define a function, given title & abstract, searches our corpus for relevant (similar) papers
def search_papers(news_title, article_text):
  query_embedding = model1.encode(news_title +' '+ article_text, convert_to_tensor=True)
  search_hits = util.semantic_search(query_embedding, corpus_embeddings, top_k= 50)
  search_hits = search_hits[0]  #Get the hits for the first query
  i=0
  for hit in search_hits:
    i=i+1
    related_titles = non_duplicate_titles[hit['corpus_id']]
    print(i, related_titles, hit['score'])

search_papers(titles[0],documents[0])
